...
public class BackpackCosmeticListener {
    ...
    // Slots to watch — storage slot 0 (hand-held) AND chest armor slot 1
    ...
    public static class BackpackData implements Component<EntityStore> {
        ...
        @Nullable private String activeBackpackItemId = null;
        @Nullable private String originalModelPath    = null;
        ...
        @Nullable public String getActiveBackpackItemId()        { return activeBackpackItemId; }
        public void setActiveBackpackItemId(@Nullable String id) { this.activeBackpackItemId = id; }
        @Nullable public String getOriginalModelPath()            { return originalModelPath; }
    ...
    private static void onInventoryChange(@Nonnull LivingEntityInventoryChangeEvent event) {
        ...
        // Storage slot 0 — backpack held in hand/storage
        ...
        if (changed == armor) {
            if (tx.wasSlotModified(CHEST_SLOT)) {
                ...
                ItemStack oldItem   = resolveOldItemInSlot(tx, CHEST_SLOT);
                String    newItemId = itemId(newItem);
                ...
                if (oldWasBackpack || newIsBackpack) {
                    // Chest slot was used for a backpack — update backpack state
                    scheduleUpdate(player, store, ref, playerUuid, newIsBackpack ? newItemId : null);
                } else {
                    // Regular chestplate equip/unequip — reapply without touching backpack state
            ...
            // Any other armor slot (helmet, leggings, boots)
            String currentBackpackItemId = readActiveBackpackItemId(player);
            if (currentBackpackItemId == null) {
                BackpackData persisted = store.getComponent(ref, BackpackData.TYPE);
                if (persisted != null) currentBackpackItemId = persisted.getActiveBackpackItemId();
    ...
    private static void onPlayerReady(@Nonnull PlayerReadyEvent event) {
        ...
        // Prefer live inventory; fall back to persisted component
    ...
    private static void scheduleArmorReapply(@Nonnull Player             player,
        ...
        Runnable task = () -> {
            ...
            // Update active backpack map without changing the stored item ID
    ...
    private static void applyBackpack(@Nonnull Store<EntityStore> store,
        ...
        Set<Cosmetic> hiddenCosmetics = resolveHiddenCosmetics(player);
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // Armor cosmetic hiding
    // ─────────────────────────────────────────────────────────────────────────

    @Nonnull
    private static Set<Cosmetic> resolveHiddenCosmetics(@Nonnull Player player) {
        Set<Cosmetic> hidden = EnumSet.noneOf(Cosmetic.class);
        Inventory inv = player.getInventory();
        if (inv == null) return hidden;
        ItemContainer armor = inv.getArmor();
        if (armor == null) return hidden;
    ...
    /**
     * Reads the active backpack item ID from the live inventory.
     * Checks chest armor slot first (equipped backpack), then storage slot 0 (held backpack).
     */
    @Nullable
    private static String readActiveBackpackItemId(@Nonnull Player player) {
        ...
        // Chest armor slot takes priority — it's the "equipped" state
        ItemContainer armor = inv.getArmor();
        if (armor != null) {
        ...
        // Fall back to storage slot 0
        ItemContainer storage = inv.getStorage();
        if (storage != null) {
            String id = itemId(storage.getItemStack(STORAGE_SLOT));
    ...
    /**
     * Resolves the item stack that ended up in the given slot after a transaction.
     * Handles SlotTransaction, MoveTransaction, and ItemStackTransaction.
     */
    @Nullable
    private static ItemStack resolveNewItemInSlot(@Nonnull Transaction   tx,
        ...
        // Fallback: read current live state
        return container.getItemStack(targetSlot);
    }

    @Nullable
    private static ItemStack resolveOldItemInSlot(@Nonnull Transaction tx, short targetSlot) {
        if (tx instanceof SlotTransaction slotTx && slotTx.getSlot() == targetSlot)
        ...
        if (tx instanceof MoveTransaction<?> moveTx) {
            if (moveTx.getMoveType() == MoveType.MOVE_TO_SELF) {
                ...
                if (addTx instanceof ItemStackTransaction addStack) {
                    for (ItemStackSlotTransaction s : addStack.getSlotTransactions())
                        if (s.getSlot() == targetSlot) return s.getSlotBefore();
                }
            } else {
                SlotTransaction removeTx = moveTx.getRemoveTransaction();
                if (removeTx.getSlot() == targetSlot) return removeTx.getSlotBefore();
            }
        }
